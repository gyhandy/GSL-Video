...
Raw_Data_root = '/lab/andy/PycharmProjects/PurePoseGan/stargan/data/train_img_c00_10class'
Target_root ='/home2/ilab2M_pose/vae_unsup_tri/'
...
for roots, dirs, files in os.walk(Raw_Data_root):
    for i, file in enumerate(files):
        ...
        # C_path = A_path.replace(A_pose, selected_pose).replace(A_pose.replace('_', '-'), selected_pose.replace('_', '-'))
        if os.path.exists(C_path):
            # C
            if not os.path.exists(os.path.join(Target_root, B_category)):
                os.mkdir(os.path.join(Target_root, B_category))
            C_target_path = os.path.join(Target_root, B_category, '{}_C.jpg'.format(i))
            shutil.copy(C_path, C_target_path)
            # A
            if not os.path.exists(os.path.join(Target_root, A_category)):
                os.mkdir(os.path.join(Target_root, A_category))
            A_target_path = os.path.join(Target_root, A_category, '{}_A.jpg'.format(i))
            shutil.copy(A_path, A_target_path)
            # B
            B_target_path = os.path.join(Target_root, B_category, '{}_B.jpg'.format(i))
            shutil.copy(B_path, B_target_path)
        else:
            print('there is no {}]'.format(C_path))
            continue
