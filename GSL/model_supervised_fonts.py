...
class Generator_fc(nn.Module):
    ...
    def forward_origin(self, x, c):
        ...
        x = torch.cat([x, c], dim=1)
        return self.main(x)
class Generator_newG(nn.Module):
    """Generator network."""
    def __init__(self, conv_dim=64, c_dim=5, repeat_num=6):
        ...
        self.start_layers = []
        # self.start_layers.append(nn.Conv2d(3+c_dim, conv_dim, kernel_size=7, stride=1, padding=3, bias=False))
        self.start_layers.append(nn.Conv2d(3, conv_dim, kernel_size=7, stride=1, padding=3, bias=False))
        self.start_layers.append(nn.InstanceNorm2d(conv_dim, affine=True, track_running_stats=True))
        self.start_layers.append(nn.ReLU(inplace=True))
        self.start_part = nn.Sequential(*self.start_layers)
        ...
        for i in range(2):
            ...
            self.down_layers.append(nn.ReLU(inplace=True))
            '''
            add residualBlock in G
            '''
            curr_dim = curr_dim * 2
            self.down_layers.append(ResidualBlock(dim_in=curr_dim, dim_out=curr_dim))
        self.down_part = nn.Sequential(*self.down_layers)
        self.eli_pose_part = nn.Sequential(*self.start_layers, *self.down_layers)
        ...
        self.bottle_layers = []
        for i in range(repeat_num):
            self.bottle_layers.append(ResidualBlock(dim_in=curr_dim + c_dim, dim_out=curr_dim + c_dim))
        self.bottlen_part = nn.Sequential(*self.bottle_layers)

        # Up-sampling layers.
        self.up_layers = []
        for i in range(2):
            if i ==0:
                self.up_layers.append(nn.ConvTranspose2d(curr_dim+ c_dim, curr_dim//2, kernel_size=4, stride=2, padding=1, bias=False))
            else:
                self.up_layers.append(
                    nn.ConvTranspose2d(curr_dim, curr_dim // 2, kernel_size=4, stride=2, padding=1, bias=False))
        ...
        self.up_layers.append(nn.Tanh())
        self.up_part = nn.Sequential(*self.up_layers)
        self.main = nn.Sequential(*self.start_layers, *self.down_layers, *self.bottle_layers, *self.up_layers)
    ...
    def forward(self, x, c):
        ...
        x1 = self.start_part(x)
        x2 = self.down_part(x1)
        x3 = torch.cat([x2, c], dim=1)
        x4 = self.bottlen_part(x3)
        x5 = self.up_part(x4)
        return x5, x2
        # x = torch.cat([x, c], dim=1)
        # return self.main(x)
...
class Classifier_Latent(nn.Module):
    ...
    def forward(self, x):
        out_src = F.relu(self.fc1(x))
        out_src = F.relu(self.fc2(out_src))
        out_src = self.fc3(out_src)
        return out_src
