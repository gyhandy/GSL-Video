...
def findD(roots, A_path):
    exist = False
    A_pose = roots.split('/')[-1]
    A_category = A_path.split('/')[-1].split('-')[0]
    A_instance = A_path.split('/')[-1].split('-')[1]
    A_background =  A_path.split('/')[-1].split('-')[2]

    pose_info = ['c00_r01', 'c00_r02', 'c00_r03', 'c00_r04', 'c00_r06', 'c00_r07']
    pose_info.remove(A_pose)
    pose_index = random.randint(0, 4)
    selected_pose = pose_info[pose_index]
    D_root = os.path.join(Raw_Data_root, selected_pose)

    D_files = os.listdir(D_root)
    random.shuffle(D_files)
    while not exist:
        for file in D_files:
            if fnmatch.fnmatch(file, '*' + A_background + '*'):
                if not A_instance in file:
                    D_path = os.path.join(D_root, file)
                    exist = True
                    break
    return D_path

# Raw_Data_root = '/home2/ilab2M_pose/vae_unbalance_train'
# Target_root ='/home2/ilab2M_pose/vae_unbalance_ABC/'
Raw_Data_root = '/home2/ilab2M_pose/test_img_c00_10class'
Target_root ='/home2/ilab2M_pose/vae_fourswap/'
pose_info = ['c00_r01', 'c00_r02', 'c00_r03', 'c00_r04', 'c00_r06', 'c00_r07']
for roots, dirs, files in os.walk(Raw_Data_root):
    for i, file in enumerate(files):
        ...
        exist = False
        while not exist:
            exist, B_path, C_path = findBC(roots, A_path)

        D_path = findD(roots, A_path)
        # D
        D_target_path = os.path.join(Target_root, 'D', '{0}_{1}_D.jpg'.format(i, pose ))
        shutil.copy(D_path, D_target_path)

        # C
        C_target_path = os.path.join(Target_root, 'C', '{0}_{1}_D.jpg'.format(i, pose ))
        shutil.copy(C_path, C_target_path)
        # A
        A_target_path = os.path.join(Target_root, 'A', '{0}_{1}_D.jpg'.format(i, pose ))
        shutil.copy(A_path, A_target_path)
        # B
        B_target_path = os.path.join(Target_root, 'B', '{0}_{1}_D.jpg'.format(i, pose ))
        shutil.copy(B_path, B_target_path)

        if i % 1000 == 0:
            print('Have provessed {0} in pose {1} images'.format(i, pose))
