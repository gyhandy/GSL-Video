...
Target_root ='/home2/ilab2M_pose/vae_unsup/'
...
for roots, dirs, files in os.walk(Raw_Data_root):
    for i, file in enumerate(files):
        ...
        if os.path.exists(C_path):
            # C
            C_target_path = os.path.join(Target_root, 'C', '{}_C.jpg'.format(i))
            shutil.copy(C_path, C_target_path)
            # A
            A_target_path = os.path.join(Target_root, 'A', '{}_A.jpg'.format(i))
            shutil.copy(A_path, A_target_path)
            # B
            B_target_path = os.path.join(Target_root, 'B', '{}_B.jpg'.format(i))
